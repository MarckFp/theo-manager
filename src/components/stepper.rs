                    ...
                    .map(|step| {
                        ...
                        rsx! {
                            div { key: "{step}", class: "flex items-center",
                                div { class: if is_current { "flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-content font-bold" } else if is_completed { "flex items-center justify-center w-10 h-10 rounded-full bg-success text-success-content" } else { "flex items-center justify-center w-10 h-10 rounded-full bg-base-300 text-base-content" },
                                    {
                                        if is_completed {
                                            rsx! {
                                                svg {
                                ...
                                if step < props.total_steps {
                                    div { class: if is_completed { "w-12 sm:w-16 h-1 bg-success mx-2" } else { "w-12 sm:w-16 h-1 bg-base-300 mx-2" } }
                                }
                            }
                        }
                    })
